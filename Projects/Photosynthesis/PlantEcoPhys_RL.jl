...
function CommonRLInterface.reset!(env::PhotonsynEnv)
    ...
    env.internal.Esoil = 250 + 750*rand()
...
function CommonRLInterface.act!(env::PhotonsynEnv, a::Real)
    ...
    env.step += 1
    env.idx = mod1(env.idx+1, length(env.external_trajectory))
    return A
end
...
function CommonRLInterface.observe(env::PhotonsynEnv)
    ...
    # end
    hour = mod(env.idx,24)/23 # [0, 1]
    day = (env.idx ÷ 366)/365 # leap year

    observation = SA{Float32}[o_E, o_s, day, hour]
    # observation = SA{Float32}[o_E, day, hour]
    return observation
end
# CommonRLExtensions.observations(env::PhotonsynEnv) = Box(fill(-Inf32, 6*env.n_obs+2), fill(Inf32, 6*env.n_obs+2))
CommonRLExtensions.observations(env::PhotonsynEnv) = Box(fill(0f0, 4), fill(Inf32, 4))
...
function get_externaltraj()
    ...
    for row in eachrow(data[(data.YEAR .== 2020), :])
        for hour in 0:23
            ...
            Wind = row.WIND
            vpd1 = RHtoVPD(row.RHMIN, Tair)
            vpd2 = RHtoVPD(row.RHMAX, Tair)
            VPD = scale_fun(min(vpd1, vpd2), max(vpd1, vpd2), hour)        
