...
df = vcat([rcopy(plantecophys.Photosyn(VPD = x.VPD, PPFD = x.PPFD, Tleaf = x.Tair)) for x in ext_traj]...)

L = length(df.ELEAF)
y = zeros(L)
for ii in 1:L-719
    y[ii] = sum(df.ELEAF[ii:ii+719])
end
for ii in L-718:L
    y[ii] = sum(df.ELEAF[ii:L]) + sum(df.ELEAF[1:mod1(ii+719,L)])
end
plot(df.ELEAF)
plot(y)
y2 = zeros(366)
for ii in 1:366
    y2[ii] = mean(y[((24*(ii-1))+1):(24*ii)])
end
plot(y2)

Esoil = PlantEcoPhys_RL.transpiration2loss.(y2, 0.002)/1000
plot(Esoil; label=false, title="30-Day Water Usage Based on Data", xlabel="Start Date", ylabel="Water Consumed [g]")
mean(Esoil)
mean(y2)
extrema(y2)
savefig("Data Water Usage")
